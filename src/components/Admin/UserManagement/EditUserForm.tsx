...
const EditUserForm: React.FC<Props> = ({ userId, isLoading, onSubmit }) => {
        ...
        >
          {({ values }) => (
            <Form className="mt-6 w-full grid grid-cols-1 justify-items-center md:grid-cols-2 md:grid-gap-2">
              ...
              <div className="mt-4 w-7/12">
                <MySelect
                  ...
                  renderValue={(selected) =>
                    (selected as string[])
                      .map(
                        (item) => roles![values.roles!.indexOf(item)].roleName
                      )
                      .join(", ")
                  }
                >
                  {roles?.map((role) => (
                    <MenuItem key={role._id} value={role._id}>
                      <Checkbox
                        checked={values.roles!.indexOf(role._id!) > -1}
                      />
              ...
              <div className="my-8 w-4/12 md:col-span-2 h-12">
                <Button
